...
const addProduct = async (req, res) => {
  try {
    const { name, category, price, img, colours, sizes, description, details } = req.body;
...
const updateProduct = async (req, res) => {
  try {
    ...
    const updatedProduct = await Product.findOneAndUpdate(
      { id: Number(id) },
      { $set: { name, category, price: Number(price), img, colours: colours || [], sizes: sizes || [], description: description || '', details: details || [] } },
      { new: true }
    );
